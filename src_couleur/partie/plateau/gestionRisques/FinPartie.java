...
public class FinPartie extends Danger {
    ...
    public Boolean isPartieTerminee()
    {
        ...
        {
            System.out.println("Pat !" + Couleur.getVert() + " Match nul\n" + Couleur.getStopCouleur());
            return true;
        }
        return false;
    }
    ...
    private boolean checkManqueMateriel()
    {
        ...
        if (piecesRestantes.size() == 3)
        {
            for (IPiece piece : piecesRestantes)
            {
                ...
                {
                    System.out.println("Manque de matériels !" + Couleur.getVert() + " Match nul\n" + Couleur.getStopCouleur());
                    return true;
                }
            }
        }
    ...
    private void affichageMat()
    {
        ...
        if (getCouleurJoueur() == CouleurPiece.BLANC)
            System.out.print(Couleur.getBleu() + "noirs");
        else
            System.out.print(Couleur.getJaune() + "blancs");
        System.out.println(Couleur.getStopCouleur() + " ont gagné\n");
    }
    ...
    private void affichageEchec(ArrayList<Coordonnees> tableauMenacePar)
    {
        for (Coordonnees coordPiece : tableauMenacePar)
        {
            ...
            if (getCouleurJoueur() == CouleurPiece.BLANC)
                System.out.print(Couleur.getBleu());
            else
                System.out.print(Couleur.getJaune());
            System.out.println(echiquier[coordPiece.getIdxLigne()][coordPiece.getIdxColonne()].getNomComplet() + Couleur.getStopCouleur() +
